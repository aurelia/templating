...
A single bindable property can be the default among all the options in an options binding.  Thus when we use a custom attribute that would otherwise require that we use the options HTML syntax, we can use the simpler HTML syntax of a single value binding. 
