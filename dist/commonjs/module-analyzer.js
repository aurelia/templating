...
var _Metadata$ResourceType = require('aurelia-metadata');
...
var ResourceDescription = function ResourceDescription(key, exportedValue, allMetadata, resourceTypeMeta) {
  ...
  if (!resourceTypeMeta) {
    if (!allMetadata) {
      allMetadata = _Metadata$ResourceType.Metadata.on(exportedValue);
    }

    resourceTypeMeta = allMetadata.first(_Metadata$ResourceType.ResourceType);
